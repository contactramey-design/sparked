...
- **AI Tutor:** `src/ai-tutor/*`, routes `/ai-tutor`; backends `api/tutor-chat.js`, **`api/liveavatar-session.js`** (LiveAvatar), `api/tts-stream.js`. Legacy `api/heygen-streaming-token.js` retained but unused by the app. See `docs/AI-TUTOR-ARCHITECTURE.md`.
