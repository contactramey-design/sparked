...
type ForSchoolsTabId = 'pilot' | 'program' | 'pricing' | 'resources' | 'compliance'

function isForSchoolsTabId(v: string | null): v is ForSchoolsTabId {
  return v === 'pilot' || v === 'program' || v === 'pricing' || v === 'resources' || v === 'compliance'
}
...
const ForSchoolsPage: React.FC = () => {
  ...
  useEffect(() => {
    if (tabParam !== null && !isForSchoolsTabId(tabParam)) {
      setSearchParams({ tab: defaultTab }, { replace: true })
    }
  }, [tabParam, defaultTab, setSearchParams])
  ...
  useEffect(() => {
    if (location.hash !== '#school-compliance') return
  ...
  return (
    <div className="page page-narrow print-page">
      ...
      <Tabs
        ...
        value={activeTab}
        onValueChange={(v) => setActiveTab(v as ForSchoolsTabId)}
        className="for-schools-tabs no-print"
      >
        <TabsList className="w-full justify-start flex-wrap h-auto gap-1 bg-amber-100/50 p-1 rounded-xl mb-4">
          <TabsTrigger value="pilot">{t('forSchoolsHub.tabPilot')}</TabsTrigger>
          <TabsTrigger value="program">{t('forSchoolsHub.tabProgram')}</TabsTrigger>
          <TabsTrigger value="pricing">{t('forSchoolsHub.tabPricing')}</TabsTrigger>
          <TabsTrigger value="resources">{t('forSchoolsHub.tabResources')}</TabsTrigger>
          <TabsTrigger value="compliance">{t('forSchoolsHub.tabCompliance')}</TabsTrigger>
        </TabsList>

        <TabsContent value="pilot" className="stack-lg mt-0">
        ...
        <TabsContent value="program" className="stack-lg mt-0">
          <Card>
            <CardHeader>
        ...
        <TabsContent value="pricing" className="stack-lg mt-0">
          <Card>
            <CardHeader>
        ...
        <TabsContent value="resources" className="stack-lg mt-0">
          <Card>
            <CardHeader>
        ...
        <TabsContent value="compliance" className="stack-lg scroll-mt-24 mt-0">
          <div id="school-compliance">
            <h2 className="text-xl font-bold">{t('compliance.title')}</h2>
            <p className="muted">{t('compliance.kidIntro')}</p>
            <ComplianceContent />
          </div>
        </TabsContent>
      </Tabs>
