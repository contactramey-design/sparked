...
type Props = {
  /** Path passed to Stripe returnTo (allowlisted server-side). */
  checkoutReturnPath?: string
}

/**
 * Tutor shell: config fetch, Stripe return handling, InteractiveTutor.
 * Used on `/tutor` (full funnel) and can be reused elsewhere.
 */
export function TutorApp({ checkoutReturnPath = '/tutor' }: Props) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (returnTo && (returnTo.startsWith('/ai-tutor') || returnTo.startsWith('/tutor'))) {
